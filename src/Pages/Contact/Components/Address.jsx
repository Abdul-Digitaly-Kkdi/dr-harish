      ...
      <div className="relative z-10 max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-6 justify-items-center">
        {stats.map((item, index) => (
          <div
            key={index}
            onClick={item.action}
            className="cursor-pointer bg-accent-gold/80 backdrop-blur-md rounded-2xl p-6 shadow-xl text-center transition transform hover:-translate-y-1 hover:shadow-2xl duration-300 w-full sm:w-80"
          >
            {item.icon}
